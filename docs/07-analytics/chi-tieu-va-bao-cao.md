...
> **Mục tiêu:** Biến dữ liệu ở kho phân tích thành hiểu biết dùng được cho nghiệp vụ, hỗ trợ ra quyết định và Machine Learning.

## 1. Từ điển chỉ tiêu

**Nguyên tắc:** Câu hỏi nghiệp vụ có trước → chỉ tiêu → công thức → dữ liệu cần → bảng cần dựng.
Làm ngược lại (nhìn bảng có gì rồi vẽ chart) sẽ ra hàng chục báo cáo không ai dùng.
...
| chỉ tiêu | Định nghĩa | Công thức | Nguồn | Lưu ý độ hạt |
|---|---|---|---|---|
...
| **ATV** (Average Ticket Value) | Giá trị hoá đơn bình quân | `SUM(net_amount) / COUNT(DISTINCT invoice_no)` | `fact_sales_line` | **Phải** có `DISTINCT` — độ hạt là dòng hoá đơn, không phải hoá đơn |
...
| chỉ tiêu | Định nghĩa | Công thức | Nguồn |
|---|---|---|---|
| **Booking Conversion** | Tỷ lệ xem → đặt | Tổng hợp **từng fact về cùng độ hạt `ngày × salon`** rồi mới chia: `booking_cnt / session_cnt`. **Không** chia trực tiếp giữa hai fact khác độ hạt | `agg_funnel_daily` |
...
| **Therapist Năng suất** | Tỷ lệ giờ KTV có khách | `SUM(busy_minutes) * 1.0 / NULLIF(SUM(available_minutes), 0)` — cả hai cùng đơn vị phút; chia phút cho giờ làm kết quả phồng 60 lần | `fact_treatment` + lịch làm việc (**chưa có nguồn**) |
...
| **Nhóm khách theo tháng đến lần đầu Retention** | Giữ chân theo nhóm tháng đầu | Ma trận nhóm theo tháng đến lần đầu × tháng thứ N | `agg_cohort_retention` |
...
```
Con số 90 ngày chỉ là **giá trị khởi tạo tạm**; phải thay bằng kết quả truy vấn trên và **rà lại mỗi 6 tháng**. Ghi rõ ngưỡng đang dùng vào từ điển dữ liệu.
...
|---|---|---|---|---|
| 1 | **Descriptive** (mô tả) | Chuyện gì đã xảy ra? | Doanh thu tháng 7 là 4,2 tỷ | Báo cáo |
...
| 4 | **Prescriptive** (đề xuất) | Nên làm gì? | Gửi voucher cho 850 khách nguy cơ rời | ML + quy tắc |

### Bộ Báo cáo

| Báo cáo | Người dùng | Nội dung chính | Tần suất |
|---|---|---|---|
| **Executive Overview** | CEO, COO | Revenue, Margin, so cùng kỳ, xếp hạng salon | Hằng ngày |
| **Salon Performance** | Quản lý salon | Doanh thu, lấp buồng, khách không đến, CSAT theo chi nhánh | Hằng ngày |
| **Service & Product Mix** | Sản phẩm | Dịch vụ bán chạy, gắn kèm, biên lãi | Hằng tuần |
| **Therapist Productivity** | Vận hành, HR | Năng suất, doanh thu/KTV, điểm đánh giá | Hằng tuần |
| **Customer & Retention** | CRM, Marketing | Nhóm khách theo tháng đến lần đầu, khách rời bỏ, CLV, phân khúc RFM | Hằng tuần |
| **Marketing ROI** | Marketing | CAC, ROAS theo kênh, hiệu quả chiến dịch | Hằng ngày |
| **chất lượng dữ liệu Ops** | Bộ phận Dữ liệu | Trạng thái DQ, số dòng vùng cách ly, độ trễ | Hằng ngày |
| **Live Salon Monitor** | Lễ tân, quản lý | Khách trong salon, buồng trống, doanh thu tạm tính | Real-time |

**Chuẩn tối thiểu cho mọi báo cáo:**
- Ghi rõ **thời điểm cập nhật cuối** ("Dữ liệu đến 06:40, 14/08/2026").
- Ghi rõ **định nghĩa chỉ tiêu** (tooltip hoặc trang chú thích), khớp với catalog.
...
| 3 | **Next Best Service** | Nên gợi ý dịch vụ nào? | Gợi ý | Dịch vụ mua ở lần kế tiếp | Chuỗi dịch vụ đã dùng, loại da, mùa | Gợi ý trong app, kịch bản bán thêm tại chỗ |
| 4 | **No-show Prediction** | Lịch hẹn nào có nguy cơ khách không đến? | Phân loại nhị phân | Đã khách không đến | Lịch sử khách không đến, thời gian đặt trước, khung giờ, thời tiết | Nhắc thêm, overbook có kiểm soát |
...
**1. Rò rỉ dữ liệu từ tương lai:** dùng thông tin của tương lai để dự đoán tương lai.
Ví dụ: dự đoán khách rời bỏ mà lại đưa vào feature "tổng chi tiêu cả năm" — trong đó có cả chi tiêu **sau** mốc dự đoán. Model sẽ đạt độ chính xác 95% khi thử nghiệm và thất bại hoàn toàn khi lên production.
